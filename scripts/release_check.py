#!/usr/bin/env python3
"""Canonical read-only release-readiness checks for Sovereign AI SOC."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--full",
        action="store_true",
        help="Compatibility flag; full validation is now the default.",
    )
    parser.add_argument(
        "--skip-frontend-build",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-pytest",
        action="store_true",
        help="Skip the backend pytest suite.",
    )
    parser.add_argument(
        "--skip-docker-build",
        action="store_true",
        help="Confirm Docker image builds are skipped; builds are never run by default.",
    )
    parser.add_argument(
        "--write-report",
        action="store_true",
...
def redact_secret_text(text: str) -> str:
    ...
    return KEY_VALUE_SECRET_PATTERN.sub(
...
def documentation_checks() -> list[Check]:
    required = (
        REPO_ROOT / "README.md",
        ...
        REPO_ROOT / "docs" / "docker-demo-packaging.md",
        REPO_ROOT / "docs" / "product" / "demo-guide.md",
        REPO_ROOT / "docs" / "operations" / "docker-demo-packaging.md",
    )
    ...
    checks = [
        Check(
            ...
            (
                ...
                if missing
                else "Required release, install, demo, packaging, and CI files are present."
            ),
            0.0,
            bool(missing),
        )
    ...
    content_requirements = (
        ("README installation link", "README.md", "INSTALL.md"),
        ("README packaging command", "README.md", "package-validate"),
        ("INSTALL release command", "INSTALL.md", "release-check"),
        ("Demo guide release command", "docs/product/demo-guide.md", "release-check"),
        (
            "Packaging guide release command",
            "docs/operations/docker-demo-packaging.md",
            "release-check",
        ),
    )
    for name, relative, marker in content_requirements:
        ...
        checks.append(
            Check(
                name,
                "Documentation",
                STATUS_OK if present else STATUS_FAIL,
                None,
                None,
                (
                    f"{relative} documents {marker}."
                    if present
                    else f"{relative} must document {marker}."
                ),
                0.0,
                not present,
            )
...
def documentation_check() -> Check:
    """Compatibility wrapper for callers expecting the original aggregate check."""
    ...
    if failures:
        return Check(
            ...
            None,
            None,
            "; ".join(check.summary for check in failures),
            0.0,
            True,
        )
    return Check(
        ...
        None,
        None,
        "Required release documentation and command references are present.",
        0.0,
    )
...
def installability_checks(runner: Runner = run_command) -> list[Check]:
    wrapper = str(REPO_ROOT / "ai-soc")
    checks = [
        command_check(
            name="Guided installer dry run",
            category="Installability",
            args=[wrapper, "install", "--profile", "demo", "--dry-run"],
            runner=runner,
            timeout=300,
        ),
        command_check(
            name="Environment initialization dry run",
            category="Installability",
            args=[wrapper, "init", "--profile", "demo", "--dry-run"],
            runner=runner,
            timeout=60,
        ),
    ]
    for relative in ("requirements.txt", "INSTALL.md"):
        path = REPO_ROOT / relative
        checks.append(
            Check(
                f"Installability file: {relative}",
                "Installability",
...
def packaging_checks(
    *,
    ...
    checks = [
        command_check(
            name="Docker packaging validation",
            category="Packaging",
            args=[wrapper, "package-validate", "--json"],
            runner=runner,
            timeout=180,
            classify=json_classifier(
                {
                    "DOCKER_PACKAGING_READY": STATUS_OK,
                    "DOCKER_PACKAGING_READY_WITH_WARNINGS": STATUS_WARN,
                    "DOCKER_PACKAGING_NOT_READY": STATUS_FAIL,
                }
            ),
        )
    ]
    checks.append(
        Check(
            ...
            "Packaging",
            STATUS_SKIP,
            f"{command_display([wrapper, 'package-validate', '--build'])}",
            None,
            (
                "Skipped by --skip-docker-build."
                if skip_docker_build
                else "Not run by default; image builds require an explicit operator command."
            ),
            0.0,
        )
    )
    ...
    *,
    ...
    checks = [
        command_check(
            ...
            category="Demo mode",
            args=[wrapper, "demo-info", "--json"],
            runner=runner,
            strict_blocking=True,
            classify=json_classifier(
                {
                    "DEMO_INFO_READY": STATUS_OK,
                    "DEMO_INFO_READY_WITH_WARNINGS": STATUS_WARN,
                    "DEMO_INFO_NOT_READY": STATUS_WARN,
                }
            ),
        ),
        command_check(
        ...
        command_check(
            name="Demo reset dry run",
            category="Demo mode",
            args=[wrapper, "demo-reset", "--dry-run", "--json"],
            runner=runner,
            strict_blocking=True,
            classify=json_classifier(
                {
                    "DEMO_RESET_DRY_RUN_READY": STATUS_OK,
                    "DEMO_RESET_READY_WITH_WARNINGS": STATUS_WARN,
                    "DEMO_RESET_NOT_READY": STATUS_WARN,
                }
            ),
        ),
    ]
    if skip_runtime:
        checks.insert(
            ...
            Check(
                ...
                "Demo mode",
                STATUS_SKIP,
                command_display([wrapper, "demo-status", "--json"]),
                None,
                "Skipped by --skip-runtime/--no-runtime.",
                0.0,
            ),
        )
    else:
...
def validation_checks(
    ...
    *,
    ...
    if skip_pytest:
        checks.append(
            Check(
                ...
                "Python",
                STATUS_SKIP,
                f"{command_display([str(python)])} -m pytest -q",
                None,
                "Skipped by --skip-pytest.",
                0.0,
            )
        )
    else:
    ...
    compose_files = (
        REPO_ROOT / "deploy" / "demo" / "docker-compose.demo.yml",
...
def release_result(checks: list[Check], strict: bool) -> tuple[str, int]:
    ...
    if strict and any(check.status == STATUS_WARN for check in checks):
        return RESULT_NOT_READY, 1
...
def build_report(
    *,
    ...
    for check in checks:
        serialized = asdict(check)
        serialized["summary"] = redact_secret_text(str(serialized["summary"]))
        serialized_checks.append(serialized)
        grouped_checks.setdefault(check.category, []).append(serialized)

    warnings = [
        check for check in serialized_checks if check["status"] == STATUS_WARN
    ]
    failures = [
        check for check in serialized_checks if check["status"] == STATUS_FAIL
    ]
    skipped = [
        check for check in serialized_checks if check["status"] == STATUS_SKIP
    ]
    branch = str(repository.get("branch", "unknown"))
    commit = str(repository.get("commit", "unknown"))
    dirty = not bool(repository.get("clean_worktree", False))
    safe_repository = {
        **repository,
        "status_short": redact_secret_text(str(repository.get("status_short", ""))),
    }
    report: dict[str, object] = {
        ...
        "checks_by_category": grouped_checks,
...
def markdown_report(report: dict[str, object]) -> str:
    ...
    lines = [
        ...
        "",
        "> Read-only validation: no service state, database data, container, image, or model was changed.",
        "",
        "| Status | Count |",
        "| --- | ---: |",
        f"| OK | {summary['ok']} |",
    ...
    for category, category_checks in report["checks_by_category"].items():
        lines.extend(
            [
                f"## {category}",
                "",
    ...
    for title, key in (("Warnings", "warnings"), ("Failures", "failures")):
        lines.extend([f"## {title}", ""])
        items = report[key]
        if items:
            lines.extend(
                f"- `{item['category']} / {item['name']}`: {item['summary']}"
                for item in items
            )
        else:
            lines.append("- None.")
    ...
    if report["failures"]:
        lines.append("- Resolve all failed checks before publishing the release.")
    if report["warnings"]:
        lines.append("- Review warnings and rerun with `--strict` for a blocking gate.")
    if not report["failures"] and not report["warnings"]:
        lines.append("- The checked revision is ready for the release workflow.")
    lines.append("- Rerun after any code, dependency, packaging, or documentation change.")
    lines.append("")
    return "\n".join(lines)
...
def collect_checks(
    ...
    *,
    ...
    repository, checks = repository_checks(runner)
    checks.extend(core_checks(python, runner))
    checks.extend(installability_checks(runner))
    checks.extend(
        packaging_checks(
            skip_docker_build=args.skip_docker_build,
            runner=runner,
    ...
    checks.extend(demo_checks(skip_runtime=args.no_runtime, runner=runner))
    checks.extend(runtime_checks(args.no_runtime, runner))
    checks.extend(python_checks(python, runner))
    checks.extend(documentation_checks())
    checks.extend(
        validation_checks(
            python,
            skip_pytest=args.skip_pytest,
...
def main(argv: list[str] | None = None) -> int:
    ...
    repository, checks = collect_checks(args)
